...
function  DoGetUserSettings() {
    ...
    document.getElementById('BrukerClientLevel').value = "";
...
function DoSendPassword(Email) {
    ...
    if (confirm('Ønsker du å sende en passord reset til brukeren ?')) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                DoGetUserSettings();
                alert_OK('Info.','Nytt passord ble sendt !',2);
            }
        };
        xhr.send();
...
function DoUpdateUser() {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            alert_OK('Info.', xhr.responseText.toString().replaceAll('"',''), 2);
