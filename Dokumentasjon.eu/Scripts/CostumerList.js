...
var ServerURL = '../api/'
writeCookie('ClientName', 'OverLorden', 30000);
            ...
            for (var key in myObjE) {
                if (myObjE.hasOwnProperty(key)) {
                    ...
                    if (ColorLine == "f6f6f6") { ColorLine = "FFFFFF"; } else { ColorLine = "f6f6f6"; }
                      Linje += "<div id='R" + key + "'><tr style='background-color:#" + ColorLine + ";' id='list" + key + "'><td>&nbsp;<a href=\"#\"><img onclick=\'DoModifycostumer(\"" + key + "\",\"" + myObjE[key].ID + "\",\"" + myObjE[key].CostumerName + "\",\"" + myObjE[key].CostumerAdress + "\",\"" + myObjE[key].CostumerPostplace + "\",\"" + myObjE[key].CostumerPhone + "\");\' alt='' style='width:20px;' src='img/edit.gif' /></a></td><td ><a onclick=\"document.location.href='CostumerCard.html?TempUserID=" + myObjE[key].ID + "'\" href=\"#\">" + myObjE[key].CostumerName + "</a></td>  <td >" + myObjE[key].CostumerAdress + "</td> <td >" + myObjE[key].CostumerPostplace + "</td> <td >" + myObjE[key].CostumerPhone + "</td>  </tr></div>";
...
function DoPostCostumerUpdate() {
    ...
    MJson += ', "CostumerName" : "' + document.getElementById('SaveKundenavn').value.replace(/["']/g, '\'\'') + '"';
    MJson += ', "CostumerAdress" : "' + document.getElementById('SaveAdresse').value.replace(/["']/g, '\'\'') + '"';
    MJson += ', "CostumerPostplace" : "' + document.getElementById('SavePoststed').value.replace(/["']/g, '\'\'') + '"';
    MJson += ', "CostumerPhone" : "' + document.getElementById('SaveTelefon').value.replace(/["']/g, '\'\'') + '"';
