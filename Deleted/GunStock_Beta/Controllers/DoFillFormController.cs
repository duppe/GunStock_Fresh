    ...
    public class DoFillFormController : ApiController
    {
        ...
        public string Get(int id, string Vari, string BaseAKA)
        {
            ...
            else if (id == 1) // Gwt Costumer info Salg
            {
                ...
                STRsql = "SELECT id, Kundenavn, Adresse, Poststed, Telefon, KundeVapen, Updated, ByUser, Info, NotOK FROM " + BaseAKA + "_Kunder WHERE (id = " + Vari + ") ORDER BY Kundenavn FOR JSON PATH, WITHOUT_ARRAY_WRAPPER";
            }
            else if (id == 2) // Gwt Costumer VapenInfo By Serial
            { STRsql = "SELECT TOP (1) id, Kundenavn, Adresse, Info, Poststed, Telefon, KundeVapen, Updated, ByUser FROM " + BaseAKA + "_Kunder WHERE (Kundenavn LIKE '" + Vari.Split('_')[0].ToString() + "%') ORDER BY Kundenavn FOR JSON PATH, WITHOUT_ARRAY_WRAPPER"; }
            ...
            { STRsql = "SELECT TOP (1) id, Kundenavn, Adresse, Info, Poststed, Telefon, KundeVapen, Updated, ByUser FROM " + BaseAKA + "_Kunder WHERE (id LIKE " + Vari + ") ORDER BY Kundenavn FOR JSON PATH, WITHOUT_ARRAY_WRAPPER "; }
