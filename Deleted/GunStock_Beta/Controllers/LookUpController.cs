...
namespace GunStock.Controllers
{
    ...
    public class LookUpController : ApiController
    {
        ...
        public string Post([FromBody] string Jsonstring)
        {
            ...
            string SessionPassword = Json["SessionPassword"].Replace("'", "_");
...
#pragma warning restore CS0168 // The variable 'ex' is declared but never used
            ...
            else if (id == "01") // InsertWeapon.js
            {
                ...
                STRsql = "SELECT (select text from " + BaseAKA + "_Status where Status = " + BaseAKA + "_Pistoler.Status) as Status, CONVERT(varchar, DateOppdatert, 23) as DateOppdatert, CONVERT(varchar, Dato, 23) as Dato, CONVERT(varchar, Bestilt, 23) as Bestilt,  Eier, Leverandor, Serienummer, Mekanisme, Merke, Modell, Caliber, LopLengde, ID, Kunde_ID FROM " + BaseAKA + "_Pistoler WHERE (Serienummer like '" + Vari + "') FOR JSON AUTO";
            ...
            else if (id == "02") // InsertWeapon.js Distinct By ID
            {
                ...
                STRsql = "SELECT (select text from " + BaseAKA + "_Status where Status = " + BaseAKA + "_Pistoler.Status) as Status, CONVERT(varchar, DateOppdatert, 23) as DateOppdatert, CONVERT(varchar, Dato, 23) as Dato, CONVERT(varchar, Bestilt, 23) as Bestilt, Eier, Leverandor, Serienummer, Mekanisme, Merke, Modell, Caliber, LopLengde, ID, Kunde_ID FROM " + BaseAKA + "_Pistoler WHERE (ID like '" + Vari + "') FOR JSON AUTO";
