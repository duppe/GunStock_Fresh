...
namespace GunStockOne_Canari.Controllers
{
    [Route("api/Fetch")]
    [ApiController]
    public class DuppeController : ControllerBase
    {
        // GET: api/<DuppeController>
        [HttpGet]
...
#pragma warning restore CS0168 // The variable 'ex' is declared but never used
            ...
            if (id == "0") // StoreList.js
            {
                ...
                STRsql = "SELECT (select text from " + BaseAKA + "_Status where Status = " + BaseAKA + "_Pistoler.Status) as Status, CONVERT(varchar, DateOppdatert, 23) as DateOppdatert, CONVERT(varchar, Dato, 23) as Dato, CONVERT(varchar, Bestilt, 23) as Bestilt, ISNULL(Eier, 'Butikk') AS Eier, Leverandor, Serienummer, Mekanisme, Merke, Modell, Caliber, LopLengde, ID, Kunde_ID, ItemText FROM " + BaseAKA + "_Pistoler WHERE ((Serienummer like '" + Vari + "%') or (ID like '" + Vari + "%') or (Eier like '" + Vari + "%') or (Eier like '" + Vari + "%')) " + addOnSQL + " FOR JSON AUTO";
            ...
            else if (id == "02") // InsertWeapon.js Distinct By ID
            {
                ...
                STRsql = "SELECT (select text from " + BaseAKA + "_Status where Status = " + BaseAKA + "_Pistoler.Status) as Status, CONVERT(varchar, DateOppdatert, 23) as DateOppdatert, CONVERT(varchar, Dato, 23) as Dato, CONVERT(varchar, Bestilt, 23) as Bestilt, ISNULL(Eier, 'Butikk') AS Eier, Leverandor, Serienummer, Mekanisme, Merke, Modell, Caliber, LopLengde, ID, Kunde_ID, ItemText FROM " + BaseAKA + "_Pistoler WHERE (ID like '" + Vari + "') FOR JSON AUTO";
...
#pragma warning restore CS0168 // The variable 'ex' is declared but never used
                ...
                else if (SearchType > 1000) // Rapporter Multi Status
                {
                    ...
                    //Innkjøpsrapport
                    else if (SearchType == 1002) //Innkjøp 
                    {
                        // make SQLstring
                        if (Periode == "1")
                        {
                            GOTsql = " (Bestilt >= '" + Ar + "-01-01') AND (Bestilt < '" + Ar + "-03-31') ";
                        }
                        if (Periode == "2")
                        {
                        ...
                        Vari = "(Status <> '0') and (Status <> '3') and " + GOTsql;
                    ...
                    else if (SearchType == 1003)
                    {
                        ...
                        Vari = "(Status = '7') or (Status = '5') or (Status = '2') or (Status = '1') and (Status != '0') " + GOTsql;
                        /*
                         Status	Text
                            0	Bestilt
                            1	På lager
        ...
        // PUT api/<DuppeController>/5
        [HttpPut("{id}")]
        public void Put(int id, [FromBody] string value)
        {
        }

        // DELETE api/<DuppeController>/5
        [HttpDelete("{id}")]
