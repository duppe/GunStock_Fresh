...
function ReadTabCookie() {
    var json_str = localStorage.getItem("TAB");
...
function WriteTabCookie() {
    ...
    Clen = localStorage.getItem("TabTick");
...
function DoNewBrowserTab(URL, ReadOnly, Name) {
    ...
    // Check if tab exist
    var Clen = localStorage.getItem("TabTick");
    ...
    if (document.cookie.indexOf("TabTick") === -1) {
        ...
        TabTick = parseFloat(localStorage.getItem("TabTick"));
        TabTick = TabTick + 1;
    }
    ...
    btn.innerHTML = Name; // + "<img style=\"float: right; margin: 0px 15px 15px 0px; \" src=\"img/CrossON.gif\" />"; //"Tab " + localStorage.getItem("ActiveTab");
...
function BringToFront(ID, URL) {
    //var TAB = "TAB" + ID;
    if (ID == null) { ID = localStorage.getItem('ActiveTab'); }

    for (var tq = 1; tq < (parseFloat(localStorage.getItem("TabTick"))+1); tq++)
    {
...
function GetBySerial(Serial) {
    ...
    if (Serial != "") {
        ...
        var MJson = '{"Search" : "' + Serial.replaceAll(".", "§").replaceAll("+", "£") + '", "SessionPassword" : "' + localStorage.getItem('SessionPassword') + '", "BaseAKA" : "' + localStorage.getItem('BaseAKA') + '"}';
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                if ((myObj[0] != null) & (myObj[1] == null)) {
                    ...
                    document.getElementById('txtID').value = are.PistolID;
                    ...
                    if (are.Status == '3') { document.getElementById('Retur').hidden = false; document.getElementById('Utlever').hidden = true;}
                    if (are.Status == '2') { document.getElementById('Utlever').hidden = false; document.getElementById('Retur').hidden = true;}
                    ...
                    for (var key in myObj) {
                        if (myObj.hasOwnProperty(key)) {
                            ...
                            if (myObj[key].Eier == "Butikk") {
                                ...
                                listing += "<tr> <td ><a style=\"cursor: pointer;\" onclick='javascript:document.getElementById(\"txtSerial\").value=\"" + myObj[key].Serienummer + "\";GetBySerial(\"" + myObj[key].PistolID + "\");'>" + myObj[key].Serienummer + "</a></td> <td>" + myObj[key].Kundenavn + "</td> <td>" + myObj[key].Modell + "</td> <td>" + myObj[key].Mekanisme + "</td> </tr>";
                            }
