        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                if ((myObj[0] != null) & (myObj[1] == null)) {
                    ...
                    document.getElementById('lblStatusText').innerText = 'Våpen har status : ' + are.Status;
                    ...
                    if (are.Status == 'Utlån') { document.getElementById('Retur').hidden = false; document.getElementById('Utlever').hidden = true; } // Status 8
                    if (are.Status == 'Reservert') { document.getElementById('Utlever').hidden = false; document.getElementById('Retur').hidden = true; } // Status 2
                    if (are.Status == 'På lager' & are.Eier != 'Butikk') { document.getElementById('Utlever').hidden = false; document.getElementById('Retur').hidden = true; } // Status 1 og ikke i 'Butikk'
                    if (are.Status == 'Bestilt') { document.getElementById('DashWeponInOut').innerHTML = 'Denne er i bestilling'; }
