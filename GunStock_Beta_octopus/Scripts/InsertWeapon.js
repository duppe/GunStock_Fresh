...
function StatusDoThing(StatusValue, StatusText) {
   // alert(StatusValue + " - " + StatusText);
    document.getElementById('DivLagerLokasjon').style.display = (StatusValue == 4) ? 'block' : 'none';
...
function validate_drpDown() {
    var Failed = false;
    selects = document.getElementsByName("IsItEmty");
...
function MDoPost() {
    ...
    if (UtFylt == true) { alert_OK('Du må fylle ut alle felt !','',4); return; }
    ...
    MJson += ', "Serienummer" : "' + DoHTMLencode(document.getElementById('MtxtSerienummer').value) + '"';
    ...
    MJson += ', "ItemText" : "' + encodeURIComponent(document.getElementById('MtxtItemText').value) + '" ';
    MJson += ', "Varenummer" : "' + encodeURIComponent(document.getElementById('MtxtVarenummer').value) + '" ';
    MJson += ', "LagerLokasjon" : "' + encodeURIComponent(document.getElementById('MtxtLagerLokasjon').value) + '" ';
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            var temp = document.getElementById('MtxtSerienummer').value
            document.getElementById('MtxtSerienummer').value = "";
            CheckIfExist();
            document.getElementById('MtxtSerienummer').value = temp;
            alert_OK('Endringer Godkjent', '', 2);
            alert('Endring GodKjent !');
...
function CheckIfExist() {
    ...
    try {
        ...
        MJson += '"Status" : "1" ';
        MJson += ', "Switch" : "3" ';
        MJson += ', "Serienummer" : "' + encodeURIComponent(document.getElementById('MtxtSerienummer').value) + '"';
        MJson += ', "Mekanisme" : "' + document.getElementById('MdrpMekanisme').options[document.getElementById('MdrpMekanisme').selectedIndex].text.replace(/["']/g, '\'\'') + '"';
        MJson += ', "ByUser" : "' + readCookie('ClientName') + '"';
        MJson += ', "BaseAKA" : "' + readCookie('BaseAKA') + '"';
        MJson += ' } ';
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            if (Astr == "0") {
                ...
                document.getElementById('MtxtDateOppdatert').disabled = false;
                document.getElementById('Mtxtdate').disabled = false;
                document.getElementById('MtxtBestilt').disabled = false;
            }
...
function DoFillSerialsListing(S) {
    ...
    if (S == "") { urlD = ServerURL + "lookup/0"; }
    else
    {
        var MJson = '{"Search" : "' + encodeURIComponent(S) + '", "SessionPassword" : "' + localStorage.getItem('SessionPassword') + '", "BaseAKA" : "' + localStorage.getItem('BaseAKA') + '"}';
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            Linje += "<tr ><td>&nbsp;Valg&nbsp;&nbsp;</td><td >&nbsp;Status&nbsp;</td><td >&nbsp;Dato&nbsp;</td>   <td >Caliber</td> <td>LøpLengde</td>  <td>Mekanisme</td> </tr></thead><tbody>";
            ...
            for (var key in myObjE) {
                if (myObjE.hasOwnProperty(key)) {
                    ...
                    //Linje += "<div id='R" + key + "'><tr style='background-color:#" + ColorLine + ";' id='list" + key + "'><td>&nbsp;<a href=\"#\"><img onclick=\'DoModifyListing(\"" + myObjE[key].DateOppdatert + "\",\"" + myObjE[key].Bestilt + "\",\"" + myObjE[key].ID + "\",\"" + myObjE[key].Leverandor + "\",\"" + myObjE[key].Merke + "\",\"" + myObjE[key].Modell + "\",\"" + Caliber + "\",\"" + myObjE[key].LopLengde + "\",\"" + myObjE[key].Serienummer + "\",\"" + myObjE[key].Status + "\",\"" + myObjE[key].Mekanisme + "\",\"" + myObjE[key].ItemText + "\",\"" + Eier + "\");\' alt='' style='width:20px;' src='img/edit.gif' /></a></td><td >" + myObjE[key].Status + "&nbsp;(" + Eier + ")" + "</td><td >" + myObjE[key].Dato + "</td>    <td >" + Caliber + "</td> <td>" + myObjE[key].LopLengde + "</td>  <td >" + myObjE[key].Mekanisme + "</td> </tr></div>";
                    Linje += "<div id='R" + key + "'><tr style='background-color:#" + ColorLine + ";' id='list" + key + "'><td>&nbsp;<a href=\"#\"><img onclick=\"location.href='Gunstock.html?page=InsertWeapon.html&id="+myObjE[key].ID+"'\" alt='' style='width:20px;' src='img/edit.gif' /></a></td><td >" + myObjE[key].Status + "&nbsp;(" + Eier + ")" + "</td><td >" + myObjE[key].Dato + "</td>    <td >" + Caliber + "</td> <td>" + myObjE[key].LopLengde + "</td>  <td >" + myObjE[key].Mekanisme + "</td> </tr></div>";
...
function DoGetTheWeapon(S) {
    ...
    if (S == "") { urlD = ServerURL + "lookup/0"; }
        ...
        var MJson = '{"Search" : "' + S.replace('/', '|').replaceAll(".", "§").replaceAll("+", "£") + '", "SessionPassword" : "' + readCookie('SessionPassword') + '", "BaseAKA" : "' + readCookie('BaseAKA') + '"}';
        urlD = ServerURL + "lookup/" ;
    }
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            if (xhr.responseText != '\"[]\"') {
                ...
                DoModifyListing(myObjE[0].DateOppdatert, myObjE[0].Bestilt, myObjE[0].ID, myObjE[0].Leverandor, myObjE[0].Merke, myObjE[0].Modell, myObjE[0].Caliber, myObjE[0].LopLengde, myObjE[0].Serienummer, myObjE[0].Status, myObjE[0].Mekanisme, myObjE[0].ItemText, myObjE[0].Eier, myObjE[0].Varenummer, myObjE[0].LagerLokasjon);
                //document.location.reload();
                CheckIfExist();
...
function DoModifyListing(DateOppdatert, Bestilt, ID, lLeverandor, lMerke, lModell, lCaliber, lLopLengde, lSerienummer, lStatus, lMekanisme, ItemText, lEier, Varenummer, LagerLokasjon) {

    

    if (lCaliber == "") { lCaliber = "-"; }
    if (lLopLengde == "") { lLopLengde = "-"; }
    if (Varenummer == null) { Varenummer = "0"; }
    if (Varenummer == 'undefined') { Varenummer = "0"; }
    if (LagerLokasjon == null) { LagerLokasjon = "0"; }
    if (LagerLokasjon == 'undefined') { LagerLokasjon = "0"; }
    ...
    if (lEier != "undefined") {
        document.getElementById('MdrpEier').value = lEier;
    ...
    document.getElementById('Mtxtdate').value = DateOppdatert;
    ...
    document.getElementById('MPost').disabled = false;
    document.getElementById('MtxtItemText').value = ItemText;
    document.getElementById('MtxtVarenummer').value = Varenummer;

    if (LagerLokasjon != "undefined") {
    document.getElementById('MtxtLagerLokasjon').value = LagerLokasjon;
    } else { document.getElementById('MtxtLagerLokasjon').value = ''; }
    
    //validate_drpDown();
    StatusDoThing(document.getElementById('MdrpStatus').selectedIndex, document.getElementById('MdrpStatus').options[document.getElementById('MdrpStatus').selectedIndex].text);
...
function OnUpdated() {
    document.location.href = 'InsertWeapon.html?Snr=' + document.getElementById('MtxtSerienummer').value + '&id=' + document.getElementById('MtxtID').value;
}