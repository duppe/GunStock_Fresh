...
// Funksjon for å hente dataene fra HTML-formen innenfor en bestemt <div> og lage JSON med navnet på tekstboksene som nøkler
function lagJSON(divId) {
    var divElement = document.getElementById(divId); // Hent den spesifikke <div>en

    if (!divElement) {
        console.error("Kunne ikke finne <div> med ID: " + divId);
        return; // Avslutt funksjonen hvis <div> ikke finnes
    }

    var tekstbokser = divElement.getElementsByTagName("input");
    var textarea = divElement.getElementsByTagName("textarea");

    var data = {};

    // Gå gjennom alle tekstboksene innenfor <div> og legg dem til i JSON med id som nøkkel
    for (var i = 0; i < tekstbokser.length; i++) {
        var navn = tekstbokser[i].id; // Hent navnet på tekstboksen
        var verdi = tekstbokser[i].value; // Hent verdien fra tekstboksen
        data[navn] = verdi; // Legg til i JSON med navnet som nøkkel og verdi som verdi
    }

    // Gå gjennom textarea innenfor <div> og legg til i JSON (her antas det at det er kun ett textarea-element)
    if (textarea.length > 0) {
        var textareaNavn = textarea[0].id;
        var textareaValue = textarea[0].value;
        data[textareaNavn] = textareaValue;
    }

    // Konverter JSON til tekst og vis det (kan også sende det til serveren)
    var jsonTekst = JSON.stringify(data, null, 2);
    console.log(jsonTekst);
}

// Kjør funksjonen når en knapp blir klikket (du må legge til en knapp i HTML som kaller denne funksjonen)

// Rapoort Functions
function toggleCustomersDropdown() {
    var select = document.getElementById('drpKunde');
    var button = document.getElementById('BtnShowCustomers');

    // Toggler synligheten av dropdown
    select.hidden = !select.hidden;

    // Oppdaterer knappeteksten basert på om select er synlig
    if (select.hidden) {
        button.value = 'Velg kunde';
        if (select.options && select.options.length > 0) {
            select.options[0].selected = true;
        }
    } else {
        button.value = 'Ikke velg kunde';

        // Sjekker om "VELG Kunde" allerede eksisterer for å unngå duplikater
        var exists = Array.from(select.options).some(option => option.value === "" && option.text === "VELG Kunde");
        if (!exists) {
            var option = document.createElement('option');
            option.value = "";
            option.text = "VELG Kunde";
            option.selected = true; // Gjør dette valget til det valgte
            select.insertBefore(option, select.firstChild); // Legger til valget øverst
        }
    }
}
