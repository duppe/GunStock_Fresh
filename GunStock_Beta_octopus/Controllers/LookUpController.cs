...
namespace GunStock.Controllers
{
    ...
    public class LookUpController : ApiController
    {
        ...
        public string Post([FromBody] string Jsonstring)
        {
            ...
            if (Customer != null && Customer != "") { Customer = " and ([Eier] LIKE '" + Customer + "')"; } else { Customer = "";  };
...
#pragma warning restore CS0168 // The variable 'ex' is declared but never used
            ...
            if (id == "0") // StoreList.js
            {
                ...
                STRsql = "SELECT (select text from " + BaseAKA + "_Status where Status = " + BaseAKA + "_Pistoler.Status) as Status, CONVERT(varchar, DateOppdatert, 23) as DateOppdatert, CONVERT(varchar, Dato, 23) as Dato, CONVERT(varchar, Bestilt, 23) as Bestilt, ISNULL(Eier, 'Butikk') AS Eier, Leverandor, Serienummer, Mekanisme, Merke, Modell, Caliber, LopLengde, ID, Kunde_ID, ItemText, ISNULL(LagerLokasjon, '-') as LagerLokasjon FROM " + BaseAKA + "_Pistoler WHERE ((Serienummer like '" + Vari + "%') or (ID like '" + Vari + "%') or (Eier like '" + Vari + "%') or (Eier like '" + Vari + "%')) " + addOnSQL.Replace("%|", "%") + " FOR JSON AUTO";
            ...
            else if (id == "01") // InsertWeapon.js
            {
                ...
                STRsql = "SELECT (select text from " + BaseAKA + "_Status where Status = " + BaseAKA + "_Pistoler.Status) as Status, CONVERT(varchar, DateOppdatert, 23) as DateOppdatert, CONVERT(varchar, Dato, 23) as Dato, CONVERT(varchar, Bestilt, 23) as Bestilt,  ISNULL(Eier, 'Butikk') AS Eier, Leverandor, Serienummer, Mekanisme, Merke, Modell, Caliber, LopLengde, ID, Kunde_ID, ItemText FROM " + BaseAKA + "_Pistoler WHERE (Serienummer like '" + Vari.Replace("%","[%]") + "') FOR JSON AUTO";
            ...
            else if (id == "02") // InsertWeapon.js Distinct By ID
            {
                ...
                STRsql = "SELECT (select text from " + BaseAKA + "_Status where Status = " + BaseAKA + "_Pistoler.Status) as Status, CONVERT(varchar, DateOppdatert, 23) as DateOppdatert, CONVERT(varchar, Dato, 23) as Dato, CONVERT(varchar, Bestilt, 23) as Bestilt, ISNULL(Eier, 'Butikk') AS Eier, Leverandor, Serienummer, Mekanisme, Merke, Modell, Caliber, LopLengde, ID, Kunde_ID, ItemText, Varenummer, LagerLokasjon FROM " + BaseAKA + "_Pistoler WHERE (ID like '" + Vari + "') FOR JSON AUTO";
            ...
            { //STRsql = "SELECT (select text from " + BaseAKA + "_Status where Status = " + BaseAKA + "_Pistoler.Status) as Status, CONVERT(varchar, Dato, 23) as Dato, Leverandor, Serienummer, Mekanisme, Merke, Modell, Caliber, LopLengde, ID FROM " + BaseAKA + "_Pistoler WHERE (Status = '" + Vari + "') FOR JSON AUTO"; 
                ...
                try
                {
                    ...
                    if (Periode == "1")
                    {
                        GOTsql = "and (DateOppdatert >= '" + Ar + "-01-01') AND (DateOppdatert < '" + Ar + "-04-01') ";
                    }
                    if (Periode == "2")
                    {
...
#pragma warning restore CS0168 // The variable 'ex' is declared but never used
                ...
                if (SearchType < 1000) // Rapporter Single Status
                {
                    ...
                    STRsql = "SELECT (select text from " + BaseAKA + "_Status where Status = " + BaseAKA + "_Pistoler.Status) as Status, ISNULL(Eier, 'Butikk') AS Eier, ";
                    STRsql += "ISNULL(CONVERT(varchar, DateOppdatert, 23), '-' ) as Dato, ISNULL(CONVERT(varchar, DatoOprettet, 23), '-' ) as DatoOprettet, " + BaseAKA + "_Pistoler.ID as gID, Leverandor, Serienummer, Mekanisme, Merke, Modell, ISNULL(Caliber, '-' ) as Caliber , ISNULL(LopLengde, '-' ) as LopLengde , " + BaseAKA + "_Pistoler.ID, ";
                ...
                else if (SearchType > 1000) // Rapporter Multi Status
                {
                   ...
                   if (SearchType == 1001) { Vari = "(Status = '3')" + Customer + " " + GOTsql; }
                   

                    //Innkjøpsrapport
                    else if (SearchType == 1002) //Innkjøp DatoOprettet er blitt Bestilt
                    {
                        // make SQLstring
                        if (Periode == "1")
                        {
                            //GOTsql = " ((Bestilt >= '" + Ar + "-01-01') AND (Bestilt <= '" + Ar + "-03-31')) or ((DatoOprettet >= '" + Ar + "-01-01') AND (DatoOprettet <= '" + Ar + "-03-31')) ";
                            //GOTsql = " ((Bestilt >= '" + Ar + "-01-01') AND (Bestilt <= '" + Ar + "-03-31')) or ((DatoOprettet >= '" + Ar + "-01-01') AND (DatoOprettet <= '" + Ar + "-03-31')) ";

                            GOTsql = "DatoOprettet BETWEEN '" + Ar + "-01-01' AND '" + Ar + "-03-31 23:59:59'";
                           // GOTsql = " ((Bestilt >= '" + Ar + "-01-01') AND (Bestilt <= '" + Ar + "-03-31')) or ((DatoOprettet >= '" + Ar + "-01-01') AND (DatoOprettet <= '" + Ar + "-03-31')) ";
                        }
                        if (Periode == "2")
                        {
                            GOTsql = "DatoOprettet BETWEEN '" + Ar + "-04-01' AND '" + Ar + "-06-30 23:59:59'";
                           // GOTsql = " (Bestilt >= '" + Ar + "-04-01') AND (Bestilt < '" + Ar + "-06-30') ";
                        }
                        if (Periode == "3")
                        {
                            GOTsql = "DatoOprettet BETWEEN '" + Ar + "-07-01' AND '" + Ar + "-10-01 23:59:59'";
                           // GOTsql = " (Bestilt >= '" + Ar + "-07-01') AND (Bestilt < '" + Ar + "-10-01') ";
                        }
                        if (Periode == "4")
                        {
                            GOTsql = "DatoOprettet BETWEEN '" + Ar + "-10-01' AND '" + Ar + "-12-31 23:59:59'";
                            //GOTsql = " (Bestilt >= '" + Ar + "-10-01') AND (Bestilt <= '" + Ar + "-12-31') ";
                        }
                        if (Periode == "5")
                        {
                            GOTsql = "DatoOprettet BETWEEN '" + Ar + "-01-01' AND '" + Ar + "-12-31 23:59:59'";
                           // GOTsql = " (Bestilt >= '" + Ar + "-01-01') AND (Bestilt <= '" + Ar + "-12-31') ";
                        }
                        if (Periode == "6") //Hele Livet for dæ 
                        {  
                            GOTsql = "";
                        }

                        //Vari = "(Status <> '0') and " + GOTsql;
                        Vari = "((Status = '1') or (Status = '2') or (Status = '3') or (Status = '5') or (Status = '6') or (Status = '2') or (Status = '8')) and " + GOTsql;
                    ...
                    else if (SearchType == 1003)
                    {
                        ...
                        // make SQLstring   DateOppdatert
                        if (Periode == "1")
                        {
                            GOTsql = "and (Dato >= '1980-01-01') AND (Dato <= '" + Ar + "-03-31') ";
                        }
                        if (Periode == "2")
                        {
                            GOTsql = "and (Dato >= '1980-01-01') AND (Dato < '" + Ar + "-06-30') ";
                        }
                        if (Periode == "3")
                        {
                            GOTsql = "and (Dato >= '1980-01-01') AND (Dato < '" + Ar + "-10-01') ";
                        }
                        if (Periode == "4")
                        {
                            GOTsql = "and (Dato >= '1980-01-01') AND (Dato <= '" + Ar + "-12-31') ";
                        }
                        if (Periode == "5")
                        {
                            GOTsql = "and (Dato >= '1980-01-01') AND (Dato <= '" + Ar + "-12-31') ";
                        ...
                        Vari = "(Status = '7') or (Status = '5') or (Status = '2') or (Status = '1') or (Status = '8') and (Status != '0') " + GOTsql;
                        /*
                         Status	Text
                            0	Bestilt
                            1	På lager
                    ...
                    STRsql = "SELECT (select text from " + BaseAKA + "_Status where Status = " + BaseAKA + "_Pistoler.Status) as Status, ISNULL(Eier, 'Butikk') AS Eier, ";
                    STRsql += "ISNULL(CONVERT(varchar, Dato, 23), '-' ) as Dato, ISNULL(CONVERT(varchar, Dato, 23), '-' ) as DatoOprettet, ISNULL(CONVERT(varchar, Dato, 23), '-' ) as MottaksDato, ISNULL(CONVERT(varchar, DateOppdatert, 23), '-' ) as DateOppdatert," + BaseAKA + "_Pistoler.ID as gID, Leverandor, Serienummer, Mekanisme, Merke, Modell, ISNULL(Caliber, '-' ) as Caliber , ISNULL(LopLengde, '-' ) as LopLengde, ISNULL(LagerLokasjon, '-' ) as LagerLokasjon, ISNULL(ItemText, '-' ) as Info , " + BaseAKA + "_Pistoler.ID, ";
                    //STRsql += "ISNULL(CONVERT(varchar, DateOppdatert, 23), '-' ) as Dato, ISNULL(CONVERT(varchar, DateOppdatert, 23), '-' ) as DatoOprettet," + BaseAKA + "_Pistoler.ID as gID, Leverandor, Serienummer, Mekanisme, Merke, Modell, ISNULL(Caliber, '-' ) as Caliber , ISNULL(LopLengde, '-' ) as LopLengde , " + BaseAKA + "_Pistoler.ID, ";
